...
class TestStrategy(bt.Strategy):
    ...
    def next(self):
        ...
        support = np.mean(local_min_no_outliers)
        resistance = np.mean(local_max_no_outliers)

        diff = resistance - support
        rThreshold = resistance - (diff/6)
        sThreshold = support + (diff/6)

        maxShares = getMaxShares(self.cash, self.dataclose[0])
        minShares = 1
        ...
        # possible buy
        if not self.position:
            if len(local_min_no_outliers) >= 1 and self.dataclose[0] <= sThreshold:

                # BUY, BUY, BUY!!! (with all possible default parameters)
                self.log('BUY CREATE, %.2f' % self.dataclose[0])

                distance = (sThreshold - self.dataclose[0]) / (sThreshold - support)
                floor =  math.floor(distance * maxShares)
                numShares = min([1,floor])

                # Keep track of the created order to avoid a 2nd order
                self.order = self.buy()
                #print("Num Shares: " + str(numShares))

        # possible sell
        else:
            if len(local_max_no_outliers) >= 1 and self.dataclose[0] >= rThreshold:
...
def getMaxShares(totalCash, price):
    return totalCash // price

def calcOrderSize(supportLevel, currPrice):
    return 
...
def ticker(name):
    ...
    cerebro.addsizer(sizer.PercentSizerInt)
